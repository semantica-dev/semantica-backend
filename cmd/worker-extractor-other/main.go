...
func main() {
	...
	appLogger := logger.New("worker-extractor-other-service", cfg.LogFormat, cfg.GetSlogLevel())
	...
	if cfg.MinioEndpoint != "" && cfg.MinioAccessKeyID != "" && cfg.MinioSecretAccessKey != "" && cfg.MinioBucketName != "" {
		...
		appLogger.Error("Minio configuration is incomplete. Worker-Extractor-Other cannot function without Minio. Exiting.", "error", "incomplete Minio config")
		os.Exit(1)
	}
	...
	service := extractorother.NewExtractorOtherService(appLogger, rmqClient, minioClient) // Передаем minioClient
	...
	appLogger.Info("Starting Worker-Extractor-Other consumer...")
	wg.Add(1)
	...
	appLogger.Info("Worker-Extractor-Other service shut down gracefully.")
}
